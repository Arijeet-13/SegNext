...
from models.Convnext.segmenter import ConvNeXtSegmenter
...
class CustomTrainer(Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps):
        ...
        head_keywords = ["decode_head", "classifier", "segmentation_head", "head", "decoder"]
        ...
        for name, param in self.model.named_parameters():
            # REMOVED `if not param.requires_grad: continue` to dynamically support LISA!
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if miou > self.best_miou:
            ...
            if hasattr(model, "save_pretrained"):
                ...
                torch.save(model.state_dict(), os.path.join(self.save_dir, "pytorch_model.bin"))
...
def main():
    ...
    with open("configs/segformer_b3.yaml", "r") as f:
        cfg = yaml.safe_load(f)

    model = ConvNeXtSegmenter(
        backbone_type='tiny', # Can natively adjust config maps
